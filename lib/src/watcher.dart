...
class TestWatcher {
  ...
  Future<void> _runFlutterTest(String filePath) async {
      ...
      for (var line in lines) {
        if (line.contains(RegExp(r'^\d{2}:\d{2} \+\d+:'))) {
          ...
          if (_lastLine != null && _lastLine == content) {
            stdout.write('\r${AnsiStyles.blue(line)}');
            stdout.flush();
          } else {
    ...
    process.stderr.transform(utf8.decoder).listen((data) {
      stdout.write(AnsiStyles.red(data));
    });
    ...
    if (exitCode == 0) {
      print(AnsiStyles.green('Test passed in ${timeDifference}ms'));
    } else {
      print(AnsiStyles.red('Test failed with exit code $exitCode in ${timeDifference}ms'));
    }
